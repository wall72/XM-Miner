...
public class CXMNeuralNetworkFileSaver
{
    ...
    private CXMSetNeuralNetworkFilteredValue nfv;
    ...
    {
         nfv = new CXMSetNeuralNetworkFilteredValue();    
         uniqueValueFileSaverOpen(); 
         unique_value_cnt = temp_cnt;       
         if (t_flag)
         {          
            nfv.setFileStatus(project,f_name, c_name, l_name, true);
         }
         else
         {       
            nfv.setFileStatus(project,f_name, c_name, null, false);
         }    
         try
         {
             for (int i=1; i<row_numbers+1; i++)
             {
                 ...
                 str_value = new String(value_byte);
                 value_number = nfv.find(str_value);
                 if (value_number==0)
                 {
                     unique_value_cnt = unique_value_cnt + 1;
                     nfv.insert(str_value,unique_value_cnt);
