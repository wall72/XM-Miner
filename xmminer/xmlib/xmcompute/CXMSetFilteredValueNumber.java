...
public class CXMSetFilteredValueNumber
{
  ...
  public void setFileStatus(String project_name, String arc_name, String column_name, int i_int, boolean new_tree_create)
  {
  ...
  private void openFile()
  {
    try
    {
    	btree = new File(set_data_path+f_name+"_"+column_seq+".tdat");
    	index = new File(set_index_path+f_name+"_"+column_seq+".tidx");
    	if (new_check)
    	{
    		btree.delete();
    		index.delete();
    	...
    	data_byte_size = calByteSize(row_numbers);
    } catch (Exception e)
  ...
  public void insert(byte[] i_value, int i_num)
  {
    number = i_num;
  	if (check_start)
  	{
  ...
  private int dataFind(byte[] fnd_byte)
  {
  	  ...
  	  if (comp_1==-1)
  	  {
  		    ...
  		    if (comp_2 == 0)
  		    {
  			     return setIntValue(getNumberByte(fnd_node));
  		    }
  		    else
  		    {
  			     return 0;
      ...
      {
      	     return setIntValue(getNumberByte(root_node));
      }
  }
  ...
  private void dataInsert(byte[] ins_byte)
  {
    ...
    if (comp_1==0)
  	...
  	if (comp_1==-1)
  	{
        ...
        //ins_node = setNumberByte(ins_node);
  ...
  private byte[] setNumberByte(byte[] i_byte)
  {
  	init_int = i_byte.length - 3*data_byte_size;
  	out_byte = setDataByteValue(number);
  ...
  private byte[] getNumberByte(byte[] i_byte)
  {
  	init_int = i_byte.length - 3*data_byte_size;
  ...
  private void setNodeData(byte[] set_index, byte[] set_data, byte[] set_node)
  {
    ...
    new_node = setNumberByte(new_node);
  ...
  public int calByteSize(int i_int)
  {
    temp_int_value = i_int;
    quot_int = 0;
    mod_int = 0;
  ...
  public void setMaxIndex(int i_num)
  {
     max_index = i_num;
  }
}